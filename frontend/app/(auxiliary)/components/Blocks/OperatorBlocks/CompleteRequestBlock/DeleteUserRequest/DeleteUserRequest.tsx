...
const DeleteUserRequest: FC<PropsType> = ({request_url, changeDeleteStatus}) => {
    ...
    const deleteRequest = async (request_url: string, csrfToken: string) => {
        ...
        const response = await axiosRequestHandler(() => OperatorService.delete_request(request_url, csrfToken))
        ...
        if ((response as AxiosResponse).status === 200) {
            ...
            dispatch(setNewNotification({
                message: `Не удалось удалить заявку: ${errorResponse.message}`,
                type: "error"
            }))
        }
    }
        ...
        <div className={styles.deleteRequestWrapper}>
            {acceptDelete ? (
                <div className={styles.acceptWrapper}>
                    ...
                    <Button onClick={() => deleteRequest(request_url, csrfToken)}
                            className={styles.acceptButton}>Подтвердить</Button>
                </div>
            ) : (
