...
async def authorize_user(request: Request, session: SessionDep):
    ...
    user_payload = jwt_token.decode_jwt_token(token=user_token)
    user_phone: str = user_payload["phone"]
    user_id: str = user_payload["owner_id"]

    if not user_phone or not user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Authorization cookie's data not found")

    candidate = await crud.get_user_by_phone(session=session, phone=user_phone)

    if not candidate or str(candidate.id) != user_id:
        response = JSONResponse(status_code=status.HTTP_200_OK, content={"authorized": False})
        response.delete_cookie(key=settings.AUTH_TOKEN_KEY)
        return response
    else:
        response = JSONResponse(status_code=status.HTTP_200_OK, content={"authorized": True})
...
@router.get("/cookie_permission", summary="Set cookie permission", status_code=status.HTTP_200_OK)
